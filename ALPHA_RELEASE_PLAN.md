...
- `./pf.py` (delegates to the `pf` runner, ships an actionable install hint
  when `pf` is missing — see §4).
...
| # | Symptom | Fix |
|---|---|---|
| 4.1 | `./pf.py` prints a one-line error when the `pf` runner is missing, with no hint how to install. | Print a multi-line, actionable error with the upstream URL and the recommended pip / clone command. |
...
6. **`pf` runner bootstrap.** Either vendor `pf` here or provide a one-liner
   in `scripts/maintenance/toolchain-check.sh` that installs it.
