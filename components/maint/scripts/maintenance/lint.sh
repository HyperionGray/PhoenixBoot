...
find staging dev wip -type f \( -name '*.c' -o -name '*.h' \) 2>/dev/null | while read -r file; do
...
# Lint Python sources
find staging dev wip scripts -type f -name '*.py' 2>/dev/null | while read -r file; do
