...
class PhoenixProgressiveRecovery:
    ...
    def run_command(self, cmd, description="", check=True, capture_output=True):
        """Run a command with robust error handling using shell=False."""
        if description:
            print(f"☠ {description}")
        ...
        try:
            if capture_output:
                result = subprocess.run(args, shell=False, capture_output=True, text=True, check=check)
            ...
            if check:
                print(f"☠ Command failed: {' '.join(shlex.quote(arg) for arg in args)}")
                print(f"   Error: {e}")
                return "", str(e), e.returncode
            return "", str(e), e.returncode
        except Exception as e:
            print(f"☠ Unexpected error running: {' '.join(shlex.quote(arg) for arg in args)}")
            print(f"   Error: {e}")
            return "", str(e), 1
