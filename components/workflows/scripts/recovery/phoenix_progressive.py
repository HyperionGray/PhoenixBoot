...
class PhoenixProgressiveRecovery:
    ...
    def run_command(self, cmd: List[str], description="", check=True, capture_output=True):
        """Run a command with error handling using argument lists only."""
        if description:
            print(f"☠ {description}")
        ...
        try:
            ...
            if check:
                print(f"☠ Command failed: {' '.join(shlex.quote(part) for part in cmd)}")
                print(f"   Error: {e}")
                return "", str(e), e.returncode
            return "", str(e), e.returncode
        except Exception as e:
            print(f"☠ Unexpected error running: {' '.join(shlex.quote(part) for part in cmd)}")
            print(f"   Error: {e}")
            return "", str(e), 1
