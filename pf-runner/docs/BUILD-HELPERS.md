...
# Build and run the build system's install step
pf autobuild install=true
```
...
- `autobuild [release=true] [jobs=N] [dir=<path>] [target=<target>] [install=true]` - **Automagic builder** (auto-detect and build)
