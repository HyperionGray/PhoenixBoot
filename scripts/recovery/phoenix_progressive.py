...
class PhoenixProgressiveRecovery:
    ...
    def run_command(
        ...
        """Run a command with error handling using explicit argument lists."""
        if description:
            print(f"☠ {description}")
        ...
        try:
            ...
            if check:
                print(f"☠ Command failed: {shlex.join(cmd_list)}")
                print(f"   Error: {e}")
                return "", str(e), e.returncode
            return "", str(e), e.returncode
        except Exception as e:
            print(f"☠ Unexpected error running: {shlex.join(cmd_list)}")
            print(f"   Error: {e}")
            return "", str(e), 1
