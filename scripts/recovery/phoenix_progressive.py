...
class PhoenixProgressiveRecovery:
    ...
    def run_command(self, cmd, description="", check=True, capture_output=True, env=None):
        """Run a command with error handling."""
        if not isinstance(cmd, (list, tuple)) or not all(isinstance(part, str) for part in cmd):
            raise TypeError("cmd must be a list/tuple of string arguments")
        ...
        try:
            ...
            if check:
                print(f"☠ Command failed: {display_cmd}")
                print(f"   Error: {e}")
    ...
    def level_3_secure(self):
        ...
        if choice == "1":
            ...
            secure_env["ARGS"] = "--backup current-firmware.bin"
            self.run_command(
                ["sudo", "make", "secure-firmware-access"],
            ...
            secure_env = os.environ.copy()
            secure_env["ARGS"] = "--read suspicious-firmware.bin"
            self.run_command(
                ["sudo", "make", "secure-firmware-access"],
            ...
            if self.confirm_escalation("write clean firmware (DANGEROUS)"):
                secure_env = os.environ.copy()
                secure_env["ARGS"] = f"--write {clean_firmware}"
                self.run_command(
                    ["sudo", "make", "secure-firmware-access"],
    ...
    def level_4_vm(self):
        ...
        if not os.path.exists(recovery_image):
            if os.path.exists(base_image):
                ...
                stdout, stderr, rc = self.run_command(["sudo", "bash", "scripts/enhance_kvm_recovery.sh"])
                if rc != 0:
