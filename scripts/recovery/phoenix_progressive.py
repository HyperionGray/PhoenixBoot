...
class PhoenixProgressiveRecovery:
    ...
    def run_command(self, cmd, description="", check=True, capture_output=True):
        ...
        Accepts either a command list (preferred) or a command string.
        Command strings are parsed with shlex and executed without shell=True.
        """
        if description:
            print(f"☠ {description}")
        ...
        if isinstance(cmd, str):
            ...
            raise TypeError("cmd must be a string, list, or tuple")
        ...
        try:
            if capture_output:
                result = subprocess.run(cmd_parts, capture_output=True, text=True, check=check)
                return result.stdout, result.stderr, result.returncode
            else:
                result = subprocess.run(cmd_parts, check=check)
