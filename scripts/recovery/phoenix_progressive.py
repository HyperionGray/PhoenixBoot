...
class PhoenixProgressiveRecovery:
    ...
    def run_command(self, cmd: Union[str, Sequence[str]], description: str = "", check: bool = True, capture_output: bool = True):
        """Run a command with error handling using argv lists (no shell)."""
        if description:
            print(f"☠ {description}")
        cmd_list: List[str] = shlex.split(cmd) if isinstance(cmd, str) else list(cmd)
        ...
        try:
            ...
            if check:
                print(f"☠ Command failed: {' '.join(shlex.quote(part) for part in cmd_list)}")
                print(f"   Error: {e}")
                return "", str(e), e.returncode
            return "", str(e), e.returncode
        except Exception as e:
            print(f"☠ Unexpected error running: {' '.join(shlex.quote(part) for part in cmd_list)}")
            print(f"   Error: {e}")
            return "", str(e), 1
