...
def _configure_logger() -> logging.Logger:
    """Configure logger with a writable file target and stdout fallback."""
    ...
    log_dir_candidates = (
        ...
        Path("out/logs/phoenixguard"),
        Path("/tmp/phoenixguard"),
    )

    for log_dir in log_dir_candidates:
        try:
            log_dir.mkdir(parents=True, exist_ok=True)
            file_handler = logging.FileHandler(log_dir / "cert_inventory.log")
            file_handler.setFormatter(formatter)
            logger.addHandler(file_handler)
            logger.debug("File logging enabled at %s", log_dir)
            break
        except OSError:
            continue
...
class PhoenixGuardCertInventory:
    ...
    def run_command(
        ...
        """Run command with argument lists (no shell parsing).

        SECURITY: Commands are executed with shell=False to avoid shell
        injection and accidental argument reinterpretation.
        """
        cmd_list = [str(part) for part in cmd]
        logger.info("Running command: %s", shlex.join(cmd_list))
        try:
    ...
    def extract_cert_info(self, cert_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            openssl_base = ["openssl", "x509", "-inform", inform, "-in", cert_file]
            
            # Get certificate text info
            result = self.run_command(openssl_base + ["-text", "-noout"])
            cert_text = result.stdout
            
            # Get subject
            result = self.run_command(openssl_base + ["-subject", "-noout"])
            subject = result.stdout.strip().replace('subject=', '')
            
            # Get issuer
            result = self.run_command(openssl_base + ["-issuer", "-noout"])
            issuer = result.stdout.strip().replace('issuer=', '')
            
            # Get fingerprint
            result = self.run_command(openssl_base + ["-fingerprint", "-noout"])
            fingerprint = result.stdout.strip().replace('SHA1 Fingerprint=', '')
            
            # Get validity dates
            result = self.run_command(openssl_base + ["-dates", "-noout"])
