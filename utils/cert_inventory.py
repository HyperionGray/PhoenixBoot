...
# Set up logging
LOG_FILE = Path("/var/log/phoenixguard/cert_inventory.log")
_handlers: List[logging.Handler] = [logging.StreamHandler(sys.stdout)]
try:
    LOG_FILE.parent.mkdir(parents=True, exist_ok=True)
    _handlers.insert(0, logging.FileHandler(LOG_FILE))
except PermissionError:
    # Keep CLI/tooling usable for non-root users; stream logging still works.
    pass
...
class PhoenixGuardCertInventory:
    def __init__(self, cert_dir: str = None):
        ...
        # Best-effort only: the top-level logger already falls back to stdout.
        try:
            LOG_FILE.parent.mkdir(parents=True, exist_ok=True)
        except PermissionError:
            pass
    ...
    def run_command(self, cmd: Sequence[str], check: bool = True) -> subprocess.CompletedProcess:
        """Run command with logging using argument lists only."""
        logger.info(f"Running command: {format_command(cmd)}")
        try:
