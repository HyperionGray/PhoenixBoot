...
# Set up logging with a writable fallback
LOG_DIR = Path(os.getenv("PHOENIXGUARD_LOG_DIR", "/var/log/phoenixguard"))
...
try:
    ...
    # Non-root users may not be able to write to /var/log; stdout logging remains enabled.
    pass
...
class PhoenixGuardCertInventory:
    ...
    def run_command(
        self, cmd: Union[str, Sequence[str]], check: bool = True
    ) -> subprocess.CompletedProcess:
        """Run command with logging using shell=False."""
        cmd_args = shlex.split(cmd) if isinstance(cmd, str) else list(cmd)
        logger.info("Running command: %s", shlex.join(cmd_args))
        try:
